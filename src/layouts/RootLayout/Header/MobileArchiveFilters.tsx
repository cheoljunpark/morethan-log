...
const MobileArchiveFilters: React.FC = () => {
  ...
  const visibleYears = showAllYears ? years : years.slice(0, VISIBLE_COUNT)
  ...
  return (
    <StyledWrapper>
      <FilterGroup
        title={language === "ko" ? "연도" : "Year"}
        items={visibleYears.map((year) => ({
          key: year,
          label: year,
      ...
      {years.length > VISIBLE_COUNT && (
        <MoreButton
          ...
          hiddenCount={years.length - VISIBLE_COUNT}
...
export default MobileArchiveFilters
